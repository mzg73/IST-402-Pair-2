...
def codebookLookup(xor, codebook, encrypt=True):
    lookupValue = None
    j = 0 if encrypt else 1
    for i in range(4):
        if codebook[i][j] == xor:
            lookupValue = codebook[i][1-j]
            break
    return lookupValue
...
# Decryption
    ...
    lookupValue = 0
    for i in range(4):
        xor = codebookLookup(ciphertext[x], typical_codebook, False)
        if x == 0:
            plaintextValue = xor ^ iv
        else:
            plaintextValue = xor ^ lookupValue

        plaintext.append(plaintextValue)
        print("The deciphered value of " + str(bin(ciphertext[x])) + " is", bin(plaintextValue)[2:])
        lookupValue = ciphertext[x]
        x += 1
...
def CFB_Mode():
    ...
    for i in range(4):
        xor = message[x] ^ lookupValue                                                              # This is considered as our "cipher text"
...
# Decryption
    x = 0
    lookupValue = codebookLookup(iv, typical_codebook)
    for i in range(4):
        xor = ciphertext[x] ^ lookupValue
        lookupValue = codebookLookup(ciphertext[x], typical_codebook)
        print("The deciphered value of " + str(bin(ciphertext[x])) + " is", bin(xor)[2:])
        x += 1
